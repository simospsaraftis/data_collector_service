...
Να αναφέρουμε εδώ, ότι για να μπορέσει το εικονικό εργαστήριο να εγκαταστήσει και να εκτελέσει όλες τις υπηρεσίες που δύναται να παρέχει στους χρήστες, θα πρέπει να επιλέξουμε να δημιουργήσουμε τουλάχιστον δύο instances, δηλαδή το σμήνος να αποτελείται τουλάχιστον από έναν master και έναν worker.<br/>
...
Η συγκεκριμένη υπηρεσία παρέχει ένα replica set, δηλαδή ένα σύνολο που αποτελείται από τρια [MongoDB](https://www.mongodb.com/) instances, ένα primary και δύο secondary, τα οποία διατηρούν το ίδιο σύνολο αρχείων. Δημιουργώντας πολλαπλά αντίγραφα των δεδομένων σε διαφορετικά instances, το replica set προσφέρει το πλεονέκτημα ότι εάν το primary instance για κάποιο λόγο δεν είναι διαθέσιμο, κάποιο από τα secondary instances θα εκλεγεί να πάρει τη θέση του και έτσι δεν θα υπάρξει θέμα. Το replica set αυτό, θα χρησιμοποιηθεί για την αποθήκευση των συμβάντων που συμβαίνουν στους τερματικούς σταθμούς του σμήνους.
